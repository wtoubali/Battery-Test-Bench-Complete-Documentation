...
enum EtatSysteme {
 ...
 unsigned long debutRelaxation = 0;
void setup() {
    Serial.begin(9600);
    lcd.begin(16, 2);
       ...
       case PHASE_CHARGE:
            ...
            if (enPauseOCV && (millis() - debutPauseOCV >= 200)) {
                ...
                if (loadvoltage >= 4.10) { // Fin de charge
                    digitalWrite(MOSFET2, LOW);
                    digitalWrite(MOSFET1,LOW);
                    etatActuel = PHASE_RELAXATION;
                    tempsCommutation = millis();
                    debutRelaxation = millis();
                    lcd.clear();
                    break;
                } else {
            ...
            break;
       case PHASE_RELAXATION: {
            
              digitalWrite(MOSFET1, LOW);
              digitalWrite(MOSFET2, LOW);

             
             unsigned long tempsEcoule = millis() - debutRelaxation;
             unsigned long tempsRestant = (tempsEcoule < 10000) ? (10000 - tempsEcoule) / 1000 : 0;
            ...
            if (millis() - dernierChronoLCD >= 500) {
              dernierChronoLCD = millis();
              lcd.setCursor(0, 0);
              lcd.print("PHASE REPOS");
              lcd.setCursor(0, 1);
              lcd.print("V: "); lcd.print(tensionAVide); lcd.print("V  ");
              lcd.print("T: ");  lcd.print(tempsRestant); lcd.print("s   ");
             }
            break;
      }
        case PHASE_DECHARGE:
            ...
            if (millis() - dernierChangementEcran >= 3000) {
                ...
                lcd.setCursor(0, 1);
                if (afficherTemp) {
                    lcd.print("Pourcentage:"); lcd.print(pourcentageBatterie); lcd.print("%"); 
                    //lcd.print("C T2:"); lcd.print(temp2, 1); lcd.print("C");
                } else {
