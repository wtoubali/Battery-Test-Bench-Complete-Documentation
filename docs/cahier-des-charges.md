...
| **Type de charge de décharge** | Résistance de puissance $5\,\Omega$ / $10\text{ W}$ avec commutation par relais |
...
* **Tension minimale de coupure ($V_{min}$) :** $2,8\text{ V}$ (Ouverture du relais pour stopper la décharge et préserver la cellule)
...
1. **Coupure Automatique :** Déconnexion physique immédiate et galvanique de la charge via l'ouverture du relais dès qu'un seuil de sécurité est atteint.
...
3. **Fiabilité :** Gestion propre des déconnexions intempestives de capteurs ou de la batterie, sécurité "Normalement Ouvert" (NO) au démarrage.
